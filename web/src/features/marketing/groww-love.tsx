...
/**
 * The credit. A market line draws itself across the section — jagged,
 * rising, like every chart in the product — and where it ends, a heart
 * blooms in Groww's own blue and mint. The only place on the site that
 * borrows Groww's palette; everywhere else stays ours.
 */

const GROWW_BLUE = "#5367FF";
const GROWW_MINT = "#00F3BB";

export function GrowwLove() {
  ...
  return (
    <section aria-label="Built for the Groww hackathon" className="mx-auto w-full max-w-[1200px] px-5 pb-28 pt-24 sm:px-8">
      <div ref={ref} className="mx-auto flex max-w-[760px] flex-col items-center text-center">
        <span className="inline-flex h-8 items-center gap-2.5 rounded-full bg-ink px-4 text-[11px] font-bold uppercase tracking-[0.08em] text-white">
          <span className="size-1.5 rounded-full" style={{ background: GROWW_MINT }} aria-hidden />
          Built for the Groww hackathon
        </span>

        {/* The line that ends in a heart. */}
        <svg viewBox="0 0 720 130" fill="none" className="mt-10 h-auto w-full max-w-[640px]" aria-hidden>
          <defs>
            <linearGradient id="groww-love-line" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0" stopColor={GROWW_BLUE} />
              <stop offset="1" stopColor={GROWW_MINT} />
            </linearGradient>
            <linearGradient id="groww-love-heart" x1="0" y1="0" x2="1" y2="1">
              <stop offset="0" stopColor={GROWW_BLUE} />
              <stop offset="1" stopColor={GROWW_MINT} />
            </linearGradient>
          </defs>
          <motion.path
            d="M8 112 L84 96 L152 104 L232 78 L308 88 L388 60 L456 70 L528 46 L586 52 L640 34"
            stroke="url(#groww-love-line)"
            strokeWidth="2.5"
          ...
          />
          {/* Bloom, then a quiet heartbeat. The plain <g> owns placement so
              motion's scale transforms never overwrite it. */}
          <g transform="translate(618 4) scale(1.9)">
            <motion.g
              initial={reduce ? { scale: 1, opacity: 1 } : { scale: 0, opacity: 0 }}
              animate={shown ? { scale: 1, opacity: 1 } : undefined}
              transition={{ delay: reduce ? 0 : 1.35, type: "spring", stiffness: 260, damping: 14 }}
              style={{ transformBox: "fill-box", transformOrigin: "center" }}
            >
              <motion.path
                d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"
                fill="url(#groww-love-heart)"
                animate={shown && !reduce ? { scale: [1, 1.09, 1] } : undefined}
                transition={{ delay: 2.2, duration: 1.1, repeat: Infinity, repeatDelay: 1.6, ease: "easeInOut" }}
                style={{ transformBox: "fill-box", transformOrigin: "center" }}
              />
            </motion.g>
          </g>
        ...
        <motion.h2
          ...
          transition={{ delay: reduce ? 0 : 1.7, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          className="display mt-8 text-balance text-[40px] leading-[1.02] sm:text-[56px]"
        >
          Sai Bhavani <span className="serif-accent text-text-2">loves</span>{" "}
          <GrowwLogo className="inline-block h-[0.78em] w-auto translate-y-[0.06em]" />
        </motion.h2>

        <motion.p
          initial={reduce ? false : { opacity: 0, y: 16 }}
          animate={shown ? { opacity: 1, y: 0 } : undefined}
          transition={{ delay: reduce ? 0 : 1.9, duration: 0.7, ease: [0.16, 1, 0.3, 1] }}
          className="mt-5 max-w-[52ch] text-[16px] leading-relaxed text-text-2"
        >
          Since was designed and shipped end to end for the Groww hackathon — a love letter to the app a generation opens
          before breakfast. Every chart in it rises the way this one does.
        </motion.p>
      </div>
    </section>
  );
...
/** The official mark, from Groww's own CDN, paths untouched. */
function GrowwLogo({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 150 50" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} role="img" aria-label="Groww">
